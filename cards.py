     ...
     def __init__(self, filename):
         ...
         for i in data:
             ...
             if not i.has_key('type') or i['type'] != 'Question':
                 logger.error('bad kind of question, skip entry {0}'.format(entryCount))
                 continue
             if not i.has_key('value'):
                 logger.error('no value, skip entry {0}'.format(entryCount))
                 continue
     ...
     def __init__(self, filename):
         ...
         for i in data:
             ...
             if not i.has_key('type') or i['type'] != 'Answer':
                 logger.error('bad kind of responde, skip entry {0}'.format(entryCount))
                 continue
             if not i.has_key('value'):
                 logger.error('no value, skip entry {0}'.format(entryCount))
                 continue
